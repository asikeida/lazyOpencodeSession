...
type Texts struct {
	...
	FieldSearchSources    string
	SearchSourceTitle     string
	SearchSourceMessage   string
...
func NewTexts(lang string) Texts {
	if ResolveLanguage(lang) == "zh-CN" {
		return Texts{
			...
			ClipboardUnavailable:  "剪贴板复制失败：%s；请手动复制：%s",
			...
			HelpLines: []HelpLine{
				...
				{Key: "搜索中 Ctrl-H/L", Description: "切换左右面板"},
				{Key: "搜索中 Ctrl-P", Description: "预览当前会话"},
				{Key: "搜索中 Ctrl-D", Description: "删除当前会话"},
			...
			FieldSearchSources:  "命中来源",
			SearchSourceTitle:   "标题",
			SearchSourceMessage: "用户消息",
			SearchSourcePath:    "路径",
	...
	return Texts{
		...
		ClipboardUnavailable:  "clipboard copy failed: %s; copy manually: %s",
		...
		MatchedUserMessages:   "Matched User Messages",
		...
		HelpLines: []HelpLine{
			...
			{Key: "Search Ctrl-H/L", Description: "Switch panes"},
			{Key: "Search Ctrl-P", Description: "Preview current session"},
			{Key: "Search Ctrl-D", Description: "Delete current session"},
		...
		FieldSearchSources:  "Match Sources",
		SearchSourceTitle:   "title",
		SearchSourceMessage: "user message",
